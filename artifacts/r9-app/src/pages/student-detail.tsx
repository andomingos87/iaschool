...
export default function StudentDetailPage() {
  ...
  const club = useMemo(
    () => (s?.clubId ? clubs.data?.find((c) => c.id === s.clubId) : undefined),
    [clubs.data, s?.clubId],
  );
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            {s.heightCm && (
              <p className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-foreground">{s.heightCm} cm</span>
              </p>
            )}
            {s.weightKg && (
              <p className="flex items-center gap-2 text-muted-foreground">
                <Weight className="size-4" />
                <span className="text-foreground">{s.weightKg} kg</span>
              </p>
            )}
            <div className="border-t border-border pt-3">
              <p className="mb-1 flex items-center gap-2 text-muted-foreground">
                <Shield className="size-4" /> Clube
              </p>
              <p data-testid="text-student-club">
                {club?.name ?? (
            ...
            <CardContent>
              {posts.isError ? (
                ...
                <div className="flex flex-col items-start gap-3">
                  <p className="text-sm text-muted-foreground">
                    Nenhum post gerado para este aluno.
                  </p>
                  {!isInTrash && (
