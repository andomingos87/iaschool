...
workspace package (`@workspace/iasport`): other artifacts depend
...
- `src/preview/brand.tsx` — Brand > Logotipo page with logo usage guidelines.
- The preview defaults to dark mode (brand default) with a light/dark toggle in
  the sidebar header.
...
`@workspace/iasport/components/...`. Never use a `@/` alias inside
...
for `@workspace/iasport` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/iasport` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/iasport/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/iasport/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/iasport/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/iasport/docs/migrating-expo.md`
