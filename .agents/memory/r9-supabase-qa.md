...
- The live Supabase DB can lag behind `artifacts/iaschool-app/supabase/setup.sql`; 404s from PostgREST usually mean a table hasn't been applied yet — schema gap, not app bug. (Full setup.sql, incl. prompt_template_versions + trigger, applied Aug 2026.)
...
- Admin tokens for API testing can be minted without a password: POST /auth/v1/admin/generate_link (magiclink) with the service key, then POST /auth/v1/verify with the hashed_token + anon key returns an access_token.
- generation_logs table + generation-logs bucket are service-role-only (RLS on, zero policies); the /admin/logs screen goes through the api-server, which requires super_admin AND the exact IAsport admin e-mail. Bucket was created via Storage API; the table needs the user to run supabase/generation-logs.sql.
