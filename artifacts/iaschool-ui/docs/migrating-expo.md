...
- Rewrite color/theme imports to
  `@workspace/iaschool-ui/lib/native-theme` and
  `@workspace/iaschool-ui/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/iaschool-ui/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/iaschool-ui` and retains only
