...
`artifacts/iaschool-ui/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/iaschool-ui/styles.css";
```
...
`node_modules/@workspace/iaschool-ui/src/components` to `content`.
...
import {
  toast,
  useToast,
} from "@workspace/iaschool-ui/hooks/use-toast";
```
...
`@workspace/iaschool-ui/components/ui/button`. Run the app's typecheck
