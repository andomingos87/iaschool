...
function useTheme(): ['dark' | 'light', (theme: 'dark' | 'light') => void] {
  const [theme, setThemeState] = useState<'dark' | 'light'>(() => {
    const stored = window.localStorage.getItem('iasport-theme');
  ...
  const setTheme = (next: 'dark' | 'light') => {
    setThemeState(next);
    window.localStorage.setItem('iasport-theme', next);
  };
        ...
        <div className="flex items-center justify-between gap-3 border-b px-5 py-5">
          <div className="min-w-0">
            <img
              src={
                theme === 'dark'
                  ? `${import.meta.env.BASE_URL}logo-color.png`
                  : `${import.meta.env.BASE_URL}logo-light.png`
              }
              alt="IAsport"
              className="h-7 w-auto"
            />
            <p className="mt-2 text-xs text-muted-foreground">Design system</p>
          </div>
          <button
            type="button"
            onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
            className="shrink-0 rounded-md border px-2 py-1 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted"
