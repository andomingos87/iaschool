...
**Atualizado em:** 20/09/2026 (M1: banco ensaiado e app adaptado; falta aplicar a migration)
...
- [!] **`SUPABASE_ACCESS_TOKEN` do `.env.local` inválido** (Unauthorized na CLI e no MCP, 20/09/2026). Sem ele o MCP `apply_migration` não roda. Gerar PAT novo em supabase.com/dashboard/account/tokens e trocar no `.env.local`. Enquanto isso, `psql` direto em `db.jtyyauivokutperouqyh.supabase.co:5432` com `SUPABASE_DB_PASSWORD` funciona (usado no ensaio). O `DATABASE_URL` do `.env.local` aponta para o pooler de transação (porta 6543), que não conhece o tenant; o de sessão (`aws-0-us-east-1`, porta 5432) funciona
...
- [~] Migration `iaschool_fase1_schools_members_classes`: SQL de referência em `artifacts/iaschool-app/supabase/fase1-min-schools-events.sql`, **ensaiado com rollback na base real em 20/09/2026** (estrutura + migração de dados semeada + triggers + RLS por sessão simulada; 2 bugs de ordem corrigidos no ensaio: drops de função antes das policies dependentes, e conversão de `role` antes de trocar a constraint). Roteiro em `supabase/rehearsal/README.md`. **Não aplicado**: falta o PAT (Transversal) e o app ainda não foi adaptado. Decisão de implementação: escola migrada nasce com `schools.id = uid` do perfil, para o prefixo `{uid}/` dos objetos já no Storage continuar válido sem mover arquivo; a aprovação de cadastro cria escola + vínculo por trigger (`ensure_school_on_approval`), então a tela de aprovações não muda
...
- [~] Ensaiar a migração ✅ (20/09/2026, ver item acima) e rodar `tests/rls.integration.test.ts` **reescrito** para o modelo por escola (pendente, recusado, órfão sem escola, isolamento A/B em 11 tabelas, admin de rede em A e B, lixeira, prefixo de Storage) — teste escrito e tipado; só roda depois de a migration ser aplicada de verdade (membro de A não lê B; admin de A e B lê as duas; `user` sem vínculo não lê nada) antes do merge
...
- [ ] **Aplicar a migration** (`apply_migration` via MCP, com PAT novo) e republicar a edge function `send-guardian-code`; depois rodar `tests/rls.integration.test.ts` e `tests/guardian-verification.integration.test.ts` contra o projeto. O app já está adaptado: é o único passo que falta para o M1 funcionar de ponta a ponta
